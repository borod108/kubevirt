...
const (
	answerFile = `<?xml version="1.0" encoding="utf-8"?>
...
const (
	...
	windowsSysprepedVMIPassword = "Gauranga" //      "Heslo123"      //
)
...
var _ = Describe("[Serial]Windows VirtualMachineInstance", func() {
	...
	BeforeEach(func() {
		...
		tests.CreatePVC(tests.OSWindows, "30Gi", "local", true)
	...
	Context("[ref_id:???]should create the Admin user as specifide in the Autounattend.xml", func() {
		...
		FIt("[test_id:??????/487]should succeed to start a vmi", func() {
			fmt.Println("yoyo here0")
			fmt.Println("#v", windowsVMI)
